...
public class ElevatorMaintenance extends Thread{
	public static int n;
	public static int m;
	public static int k;
	public static int t;
	...
	public static int cleanFloors;
	public static int lastFloors;
	...
	public static ElevatorMaintenance es;
	public static Semaphore mutex = new Semaphore(1, false);
	public static Semaphore janitor = new Semaphore(1, false);
	...
	public static boolean locke;
	
	public ElevatorMaintenance(int i, int c){
		id = i;
		cust = c;
	}
	...
	public static void main(String[] args){
		...
		es = new ElevatorMaintenance(0, 0);
		es.start();
	}
	...
	public static void ElevatorSynch(int n){
		...
		load();
		checkForSems();
		if (lastFloors >= cleanFloors){
			janitor();
		}
	}
	
	public static void janitor(){
		System.out.println("+++The elevator is in need of a cleaning!");
		if (curFloor == 0){
			locke = false;
			System.out.println("+++The cleaning button has been pushed.");
			if (cust == 0){
				try{
					janitor.acquire();
				...
				janitor.release();
				System.out.println("+++The elevator has been cleaned!");
				lastFloors = 0;
			}
		}
		...
		if (called[curFloor] && cust < k){
			if (availPeople > k - cust){
				...
				if(availPeople == k - cust){
					...
					cust = 3;
				} else if (numPeople[curFloor] < k - cust){
	...
	public static void checkForSems(){
		...
		lastFloors = lastFloors + Math.abs(check-curFloor);
		System.out.println("Floors since the elevator was last cleaned: " + lastFloors);
	}
}